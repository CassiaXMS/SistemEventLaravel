<x-guest-layout>

    <link rel="stylesheet" href="{{ asset('front/css/login-style.css') }}?v={{ time() }}">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap" rel="stylesheet">

    <div class="text-center ">
        <img src="{{ asset('/front/logo/logo3.png') }}" alt="Logo" class="mx-auto mb-4" style="max-width: 200px;">
    </div>

    <div class="mb-6">
        <h1 class="text-4xl font-semibold text-gray-800">Olá,</h1>
        <h1 class="text-4xl font-semibold text-gray-800">Tudo bem?</h1>
        <span id="span">{{ __("Novo(a) por aqui?") }}</span>
        <a href="{{ route('register') }}" class="text-blue-500">{{ __('Cadastrar') }}</a>
    </div>

    <div class="login-form-container">
        <!-- Session Status -->
        <x-auth-session-status class="mb-4" :status="session('status')" />
        <form method="POST" action="{{ route('login') }}">
            @csrf
            <!-- Email Address -->
            <div>
                <x-input-label for="email" :value="__('E-mail')" />
                <x-text-input id="email" class="block mt-1 w-full font-bold text-black" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
                <x-input-error :messages="$errors->get('email')" class="mt-2" />
            </div>

            <!-- Password -->
            <div class="mt-4">
                <x-input-label for="password" :value="__('Senha')" />
                <x-text-input id="password" class="block mt-1 w-full" type="password" name="password" required autocomplete="current-password" />
                <x-input-error :messages="$errors->get('password')" class="mt-2" />
            </div>

            <!-- Remember Me -->
            <div class="block mt-4">
                <label for="remember_me" class="inline-flex items-center">
                    <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="remember">
                    <span class="ms-2 text-sm text-gray-600">{{ __('Lembrar-me') }}</span>

                    @if (Route::has('password.request'))
                        <a class=" underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ms-4" href="{{ route('password.request') }}">
                            {{ __('Esqueci minha senha') }}
                        </a>
                    @endif


                </label>
            </div>

            <div class="flex items-center justify-center mt-4 ">
                <x-primary-button class=" ms-3 form-button text-center ">
                    {{ __('Entrar') }}
                </x-primary-button>
            </div>

            <!-- Link para a página inicial -->
            <div class="mt-4 text-center ">
                <a href="{{ url('/') }}" class="text-blue-500 hover:underline">
                    {{ __('Voltar para home') }}
                </a>
            </div>
        </form>
    </div>
</x-guest-layout>
